...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Theme and dropdown event listeners
  const themeToggle = document.getElementById('themeToggle');
  const themeIcon = document.getElementById('themeIcon');
  const favoritesButton = document.getElementById('favoritesButton');
  const favoritesMenu = document.getElementById('favoritesMenu');
  ...
  favoritesButton.addEventListener('click', (e) => {
    e.stopPropagation();
    if (favoritesMenu.classList.contains('active')) {
      closeDropdown(favoritesMenu);
    } else {
      openDropdown(favoritesMenu);
      closeDropdown(unitsMenu); // Close other dropdown
    }
  });
  ...
  unitsButton.addEventListener('click', (e) => {
    e.stopPropagation();
    if (unitsMenu.classList.contains('active')) {
      closeDropdown(unitsMenu);
    } else {
      openDropdown(unitsMenu);
      closeDropdown(favoritesMenu); // Close other dropdown
    }
  });

  // Close dropdowns when clicking outside
  document.addEventListener('click', (e) => {
    if (!favoritesMenu.contains(e.target) && !favoritesButton.contains(e.target)) {
      closeDropdown(favoritesMenu);
    }
